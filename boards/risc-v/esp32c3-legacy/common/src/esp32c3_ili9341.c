/****************************************************************************
 * boards/xtensa/esp32c3/common/src/esp32c3_ili9341.c
 *
...
#ifdef CONFIG_esp32c3_LCD_OVERCLOCK
...
static void esp32c3_ili93414ws_select(struct ili9341_lcd_s *lcd);
static void esp32c3_ili93414ws_deselect(struct ili9341_lcd_s *lcd);
static int esp32c3_ili93414ws_backlight(struct ili9341_lcd_s *lcd,
                                        int level);
static int esp32c3_ili93414ws_sendcmd(struct ili9341_lcd_s *lcd,
                                      const uint8_t cmd);
static int esp32c3_ili93414ws_sendparam(struct ili9341_lcd_s *lcd,
                                        const uint8_t param);
static int esp32c3_ili93414ws_sendgram(struct ili9341_lcd_s *lcd,
                                       const uint16_t *wd, uint32_t nwords);
static int esp32c3_ili93414ws_recvparam(struct ili9341_lcd_s *lcd,
                                        uint8_t *param);
static int esp32c3_ili93414ws_recvgram(struct ili9341_lcd_s *lcd,
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_select
 *
 * Description:
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_deselect
 *
 * Description:
 *   De-select the SPI
 *
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_backlight
 *
 * Description:
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_sendcmd
 *
 * Description:
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_sendparam
 *
 * Description:
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_sendgram
 *
 * Description:
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_recvparam
 *
 * Description:
...
/****************************************************************************
 * Name: esp32c3_ili93414ws_recvgram
 *
 * Description:
...
int board_lcd_initialize(void)
{
  ...
  if (g_lcd == NULL)
    {
      spi = esp32c3_spibus_initialize(LCD_SPI_PORTNO);
      if (!spi)
        {
      ...
      esp32c3_configgpio(GPIO_LCD_DC, OUTPUT_FUNCTION_3);
      esp32c3_gpio_matrix_out(GPIO_LCD_DC, SIG_GPIO_OUT_IDX, 0, 0);

      esp32c3_configgpio(GPIO_LCD_RST, INPUT_FUNCTION_3);
      esp32c3_gpio_matrix_out(GPIO_LCD_RST, SIG_GPIO_OUT_IDX, 0, 0);

      esp32c3_configgpio(GPIO_LCD_BCKL, OUTPUT_FUNCTION_3);
      esp32c3_gpio_matrix_out(GPIO_LCD_BCKL, SIG_GPIO_OUT_IDX, 0, 0);

      /* Reset ILI9341 */

      up_mdelay(10);
      esp32c3_gpiowrite(GPIO_LCD_RST, false);
      up_mdelay(10);
      esp32c3_gpiowrite(GPIO_LCD_RST, true);
      up_mdelay(50);
      ...
      priv->dev.select      = esp32c3_ili93414ws_select;
      priv->dev.deselect    = esp32c3_ili93414ws_deselect;
      priv->dev.sendcmd     = esp32c3_ili93414ws_sendcmd;
      priv->dev.sendparam   = esp32c3_ili93414ws_sendparam;
      priv->dev.recvparam   = esp32c3_ili93414ws_recvparam;
      priv->dev.sendgram    = esp32c3_ili93414ws_sendgram;
      priv->dev.recvgram    = esp32c3_ili93414ws_recvgram;
      priv->dev.backlight   = esp32c3_ili93414ws_backlight;
